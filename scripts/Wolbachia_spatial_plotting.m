function res = Wolbachia_spatial_plotting(intro, Nr, T, varargin)
...
if nargin > 3 && ~isempty(varargin{1})
    ...
    plot_update_dts = 1; % Default: update plot every 1 dt steps
end
...
m = 2;
A = zeros(m, Nr);
A(1, :) = K;        % wild population
genotype_names = {'w', 'd'};

%% Introduction
rr = length(intro);
A(2, 1:rr) = intro./(1-intro)*K; % Wolbachia introduction
released = A(2, :)*ring_area;
fprintf('\n');
fprintf('Number of Wolbachia individuals released: %f\n', released);
...
fprintf('Number of Wolbachia individuals harvested: %f\n', harvested);
